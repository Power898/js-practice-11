 ...
 */
function customSet(arr) {const set = new Set(arr); 
  for (const value of set){if(typeof value === "number") { set.delete(value);} } return set;  
 ...
 */
function clearSet(set) {if (set.size>0){const set1=set.clear; console.log(set1, `Множину очищено`);}
 ...
 */
function addElements(mySet, arr) { for(let item of arr){ mySet.add(item);} return mySet;
 ...
 * Повертаємо - Оновлену множину.
 */
function filterAndAdd(mySet, arr) { for(let item of mySet) { if(typeof item === "number"){ mySet.delete(item);} }

for(let item of arr){ mySet.add(item);} return mySet; }
 ...
 */
function checkValueAndType(mySet, value) {if( mySet.has(value) === true){return ` Множина має значення "${value}" типу "${typeof value}"`;} 
else if(mySet.has(value) === false){return ` Множина не має значення "${value}"`;}
 ...
 */
function setToArray(mySet) { const array = [...mySet] ; 
const Set2=array.filter((item)=> typeof item === "string"); const Set3= Set2.sort(); return Set3;
 ...
 */
function areDisjoint(set1, set2) {for (const value of set1){if(value === set2.has) {return false;} else{return true;}};
 ...
 */
function getDifference(set1, set2) {const differenceSet  = new Set(); for (const value of set1){ if(set2.has(value) !== true){ differenceSet.add(value);}} return differenceSet;}
 ...
 */
function getIntersection(arr1, arr2) { const intersectionSet  = new Set(); for (const value of arr1){ if(arr2.includes(value)===true) {intersectionSet.add(value);}} return intersectionSet ;}
 ...
 */
function iterateSet(set) { const myKeys=set.keys(); for (const key of set)
  {console.log(myKeys.next().value);} 
const iterator=set.values(); for (const value of set)
  {console.log(iterator.next().value);} 
const iterator1=set.entries(); for (const value of set)
